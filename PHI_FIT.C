...
double mass = 0.547;
double Gamma = 0.0311;
void PHI_FIT (int lb, int rb, double pt_min, double pt_max, int hist_N = 7, int meth = 0, bool smooth = true){
    char filename[200] = "input/HeAu_19829_Artem_onlyemcid.root";    ///Lambda_AuAu_V1_mp_g Lambda_AuAu_V1_pm_g.root
    char histname_bg[200], histname_fg[200];
    sprintf(histname_bg,"eta_spectrum_pbsc_hist_BG11");///"LAM_V2_BG12"eta_prime_spectrum_nopid_cut_BG12;
    sprintf(histname_fg,"eta_spectrum_pbsc_hist_FG11;1");///"LAM_V2_FG12"eta_prime_spectrum_nopid_cut_FG12;
    int NVertex = 6;
    int Nrp = 1;
    ...
    char outputname[200];
    sprintf(outputname,"output/Kstar_%.1f_%.1f.png",pt_min,pt_max);
...
//   cout<<"Total integral (bad): "<<(bwconv->GetParameter(2))/bins<<" +/- "<<(bwconv->GetParError(2))/bins<<endl;
   cout<<"Total Stats: "<<stat->GetBinContent(2)/1e6<<" *10^6"<<endl;
    ...
    sprintf(integral,"#pm%.1f%%",100*(1*sqrt( intgr1 )/intgr));
